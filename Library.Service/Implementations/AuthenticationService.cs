...
namespace Library.Service.Implementations
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<JwtAuthResult> GetJWTToken(IdUser iduser)
        {
            var (jwtToken, accessToken) =await GenerateJWTToken(iduser);
            var refreshToken = GetRefreshToken(iduser.UserName);
        ...
        private async Task<(JwtSecurityToken, string)> GenerateJWTToken(IdUser iduser)
        {
            var claims = await GetClaims(iduser);
        ...
        public async Task<List<Claim>> GetClaims(IdUser iduser)
        {
            var roles = await _userManager.GetRolesAsync(iduser);
            var claims = new List<Claim>()
            {
                new Claim(ClaimTypes.Name,iduser.UserName),
                new Claim(ClaimTypes.NameIdentifier,iduser.UserName),
                new Claim(ClaimTypes.Email,iduser.Email),
                new Claim(nameof(UserClaimModel.PhoneNumber), iduser.PhoneNumber),
                new Claim(nameof(UserClaimModel.Id), iduser.Id.ToString())
            };
            foreach (var role in roles)
            {
                claims.Add(new Claim(ClaimTypes.Role, role));
            }
            var userClaims = await _userManager.GetClaimsAsync(iduser);
            claims.AddRange(userClaims);
            return claims;
        }

        public async Task<JwtAuthResult> GetRefreshToken(IdUser iduser, JwtSecurityToken jwtToken, DateTime? expiryDate, string refreshToken)
        {
            var (jwtSecurityToken, newToken) = await GenerateJWTToken(iduser);
            var response = new JwtAuthResult();
