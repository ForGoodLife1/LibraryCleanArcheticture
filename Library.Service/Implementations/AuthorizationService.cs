...
namespace Library.Service.Implementations
{
    ...
    public class AuthorizationService : IAuthorizationService
    {
        ...
        public async Task<ManageUserRolesResult> ManageUserRolesData(IdUser iduser)
        {
            var response = new ManageUserRolesResult();
            var rolesList = new List<UserRoles>();
            //Roles
            var roles = await _roleManager.Roles.ToListAsync();
            response.UserId = iduser.Id;
            foreach (var role in roles)
            {
                var userrole = new UserRoles();
                userrole.Id = role.Id;
                userrole.Name=role.Name;
                if (await _userManager.IsInRoleAsync(iduser, role.Name))
                {
        ...
        public async Task<ManageUserClaimsResult> ManageUserClaimData(IdUser iduser)
        {
            var response = new ManageUserClaimsResult();
            var usercliamsList = new List<UserClaims>();
            response.UserId = iduser.Id;
            //Get USer Claims
            var userClaims = await _userManager.GetClaimsAsync(iduser); //edit
